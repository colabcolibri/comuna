...
| `GET` | `/api/directory/members` | Diretório rico | Member; **404 se plugin off** | `search` / `status` / `cohort` / `country` (ISO-2) / `near` (`lat,lon`) / `radius` (25–3000 km, passo 25, default 50 com `near`) / `attr.*` / `page` / `size` (24, 48, 96; default 24). `view=map`: página 1, até 5000 pins (sem pager) | `{ "data", "countries", "meta": { page, pageSize, total } }` |
| `GET` | `/api/profiles/public` | Vitrine desta comunidade | Public; **404 se plugin off** | `?slug=` ou cookie `community_slug` + `search` / `status` / `country` / `near` / `radius` / `attr.*` / `page` / `size` (24, 48, 96; default 24). `view=map` igual ao diretório | `{ "data", "countries", "meta": { page, pageSize, total } }` |
