...
| `GET` | `/api/directory/members` | Diretório rico | Member; **404 se plugin off** | `search` / `status` / `cohort` / `country` (ISO-2) / `near` (`lat,lon`) / `radius` (25\|50\|100\|250, default 50 com `near`) / `attr.*` / `page` / `size` (24, 48, 96; default 24). `view=map` é só UI | `{ "data", "countries", "meta": { page, pageSize, total } }` |
