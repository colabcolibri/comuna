...
| `/c/{slug}/directory` | Buscar membros **desta** comunidade | Membro | Sidebar: comunidade + Diretório | Path legado `/directory` | Grelha de `AppPersonCard` compacto; filtros de país/cidade/raio; `AppShowcasePager` acima e abaixo; toggle mapa se o plugin `map` estiver on; pin = foto/iniciais com agrupamento; o cartão (ou o pin) abre o mesmo diálogo da vitrine. Overlay Radix (`z-50`) fica acima do mapa porque o Leaflet não vaza z-index para o documento |
